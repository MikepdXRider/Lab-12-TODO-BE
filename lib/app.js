...
app.delete('/api/todos/:id'/* ❗the /:id is added on front end.❗ */, async(req, res) => {
  try {
...
app.put('/api/todos/:id'/* ❗the /:id is added on front end.❗ */, async(req, res) => {
  try {
    ...
    // The tables ID is sequential, meaning no two users should have a matching ID. 
    res.json(data.rows);
  } catch(e) {
